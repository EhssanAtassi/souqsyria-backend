...
export class OrderWorkflowController {
  ...
  @ApiOperation({
    ...
    description: 'Retrieves the complete workflow state and history for a specific order',
  })
  @ApiParam({
    name: 'orderId',
  ...
  async getWorkflowByOrderId(@Param('orderId') orderId: number, @Request() req) {
    this.logger.log(`Admin ${req.user?.uid} requested workflow for order ${orderId}`);
  ...
  @ApiOperation({
    ...
    description: 'Executes a specific action on a workflow, triggering state transitions and business logic',
  })
  @ApiParam({
    name: 'workflowId',
  ...
  async executeWorkflowAction(
    ...
    const workflow = await this.workflowService.getWorkflowByOrderId(workflowId);
  ...
  @ApiOperation({
    ...
    description: 'Retrieves the complete audit trail of state transitions for a workflow',
  })
  @ApiParam({
    name: 'workflowId',
    description: 'Workflow ID',
  ...
  @ApiResponse({ status: 404, description: 'Workflow not found' })
  @Permissions('order.workflow.read')
  async getWorkflowHistory(@Param('workflowId') workflowId: number, @Request() req) {
    this.logger.log(`Admin ${req.user?.uid} requested history for workflow ${workflowId}`);
  ...
  @ApiOperation({
    ...
    description: 'Retrieves comprehensive analytics and performance metrics for workflows',
  })
  @ApiQuery({
    name: 'startDate',
    required: true,
  ...
  async getWorkflowAnalytics(@Query() query: WorkflowAnalyticsDto, @Request() req) {
    this.logger.log(
    ...
    return {
      ...
      stateDistribution: [
        { state: 'pending', count: 45, percentage: 15.2, averageTimeInState: 1800000 },
        { state: 'payment_confirmed', count: 180, percentage: 60.8, averageTimeInState: 900000 },
        { state: 'shipped', count: 50, percentage: 16.9, averageTimeInState: 86400000 },
        { state: 'delivered', count: 21, percentage: 7.1, averageTimeInState: 3600000 },
      ],
      performanceMetrics: {
        bottlenecks: [
          { state: 'awaiting_vendor_approval', averageTime: 7200000, impact: 'high' },
  ...
  @ApiOperation({
    ...
    description: 'Retrieves workflows that need manual intervention, have SLA breaches, or are escalated',
  })
  @ApiQuery({
    name: 'type',
    required: false,
  ...
  async getWorkflowsRequiringAttention(
    ...
    this.logger.log(`Admin ${req.user?.uid} requested workflows requiring attention`);
  ...
  @ApiOperation({
    ...
    description: 'Executes the same action on multiple workflows simultaneously',
  })
  @ApiBody({
  ...
  async bulkExecuteWorkflowActions(
    ...
    if (dto.workflowIds.length > 100) {
      throw new BadRequestException('Maximum 100 workflows can be processed at once');
    }
