...
export class CommissionsController {
  ...
  @ApiResponse({ status: 200, description: 'Discount rule deleted successfully' })
  ...
  @ApiOperation({ 
    ...
    description: 'Processes commission calculations in batches for high-performance bulk operations'
  })
  @ApiBody({
    schema: {
      type: 'object',
  ...
  async bulkCalculateCommissions(
    ...
    this.logger.log(`Admin ${req.user?.uid} initiated bulk commission calculation for ${orderIds.length} orders`);
    
    return this.commissionsService.bulkCalculateCommissions(orderIds, batchSize);
  }
  ...
  @ApiOperation({ 
    ...
    description: 'Provides detailed commission breakdown, vendor performance, and trend analysis'
  })
  @ApiQuery({ 
    name: 'startDate', 
    required: true, 
  ...
  async getCommissionAnalytics(
    ...
    this.logger.log(`Admin ${req.user?.uid} requested commission analytics from ${startDate} to ${endDate}`);
  ...
  @ApiOperation({ 
    ...
    description: 'Checks all commission rules for consistency and identifies potential issues'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async validateCommissionConfiguration(@Request() req) {
    this.logger.log(`Admin ${req.user?.uid} initiated commission configuration validation`);
  ...
  @ApiOperation({ 
    ...
    description: 'Aggregates all commission amounts for a specific vendor across all orders'
  })
  ...
  @ApiResponse({ 
    ...
    schema: {
      type: 'object',
      properties: {
        total: { type: 'number', description: 'Total commission amount for the vendor' }
      }
    }
  })
  @ApiResponse({ status: 404, description: 'Vendor not found' })
  async calculateVendorCommission(@Param('id') vendorId: number, @Request() req) {
    this.logger.log(`Admin ${req.user?.uid} requested commission calculation for vendor ${vendorId}`);
