...
export class SyrianAddressService {
  ...
  private async initializeMajorCities(): Promise<void> {
    const damascusGov = await this.governorateRepo.findOne({ where: { code: 'DIM' } });
    const aleppoGov = await this.governorateRepo.findOne({ where: { code: 'ALE' } });
      ...
      const damascusCities = [
        {
          ...
          logistics: {
            ...
            lastMileOptions: ['standard', 'express'] as ('standard' | 'express' | 'pickup_point')[],
          },
        },
        {
          governorate: damascusGov,
          ...
          logistics: {
            ...
            lastMileOptions: ['standard', 'express', 'pickup_point'] as ('standard' | 'express' | 'pickup_point')[],
          },
        },
      ];
   ...
   */
  async getCitiesByGovernorate(governorateId: number): Promise<SyrianCityEntity[]> {
   ...
   */
  async validateAddress(addressData: Partial<SyrianAddressEntity>): Promise<AddressValidationResult> {
    const issues: string[] = [];
    let confidence = 100;
    ...
    if (addressData.latitude && addressData.longitude) {
      if (addressData.latitude < 32 || addressData.latitude > 37 ||
          addressData.longitude < 35 || addressData.longitude > 42) {
   ...
   */
  async createSyrianAddress(addressData: Partial<SyrianAddressEntity>): Promise<SyrianAddressEntity> {
    // Validate address first
    const validation = await this.validateAddress(addressData);
    
    if (!validation.isValid) {
      throw new BadRequestException(`Address validation failed: ${validation.issues?.join(', ')}`);
    }
   ...
   */
  async getDeliveryZoneInfo(governorateId: number, cityId?: number): Promise<DeliveryZoneInfo> {
    const governorate = await this.governorateRepo.findOne({
    ...
    const city = cityId ? await this.cityRepo.findOne({
      where: { id: cityId },
    }) : null;

    // Calculate delivery information
    const isSupported = governorate.status?.deliverySupported && 
    ...
    return {
      ...
      restrictions: governorate.status?.accessibilityLevel === 'limited' 
   ...
   */
  async searchAddresses(query: string, language: 'en' | 'ar' = 'en'): Promise<any[]> {
    const results = [];

    // Search governorates
    const governorateQuery = this.governorateRepo.createQueryBuilder('gov')
      .where(language === 'ar' ? 'gov.nameAr LIKE :query' : 'gov.nameEn LIKE :query', 
             { query: `%${query}%` })
    ...
    // Search cities
    const cityQuery = this.cityRepo.createQueryBuilder('city')
      .leftJoinAndSelect('city.governorate', 'gov')
      .where(language === 'ar' ? 'city.nameAr LIKE :query' : 'city.nameEn LIKE :query', 
             { query: `%${query}%` })
  ...
  private calculateDeliveryFee(governorate: SyrianGovernorateEntity, city?: SyrianCityEntity): number {
    let baseFee = 500; // SYP
